...
export const useCreateSupervisor = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateSupervisorFormData | any) => supervisorService.create(data),
    onSuccess: () => {
...
export const useUpdateSupervisor = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string, data: UpdateSupervisorFormData | any }) => supervisorService.update(id, data),
    onSuccess: () => {
